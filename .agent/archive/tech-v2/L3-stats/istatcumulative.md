# IStatCumulative · 可累积接口

> `Stats/Interfaces/IStatCumulative.cs` — 只增不减的累积能力接口契约

## 用途

标记 Stat 只能由外部被动触发增加（击杀得经验、采集得材料等）。无 Tick 自动行为，不参与自动消耗/恢复。

StatDefSO 不实现此接口，改在 Inspector 勾选 `isCumulative`。
