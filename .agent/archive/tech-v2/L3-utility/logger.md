...
> `Utility/Logger.cs` — 已标记为"待废弃"的旧 Logger。具备智能序列化能力，目前作为 ConsoleAppender 的格式化后端保留。
...
| 方向 | 模块 | 关系 |
|------|------|------|
| 被依赖 | 08-logging (ConsoleAppender) | ConsoleAppender 使用 Logger 做格式化输出 |
| 被依赖 | (旧代码) | 历史遗留的直接调用 |
...
```
- 与 `08-logging/LogLevel` 不同，此枚举只有三级
...
| 规划 | 状态 | 来源 |
|------|------|------|
| 废弃此 Logger，功能迁至 08-logging | 待做 | 架构规划 + 代码注释 |
