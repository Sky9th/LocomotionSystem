...
> `Utility/Logging/ConsoleAppender.cs` — ILogAppender 的 Unity Debug.Log 实现，将日志通过 Unity Console 输出
...
| 依赖 | Logger (旧 Logger) | 将格式化委托给 Logger.Log/LogWarning/LogError |
...
- **备注**: 委托给旧 `Logger` 类，旧 Logger 负责格式化 (添加级别标签、频道标签) 和调用 `UnityEngine.Debug.Log`

## 三级过滤在本类的位置

`ConsoleAppender.Append()` 内部做第三级过滤: `if (level < Threshold) return;`
