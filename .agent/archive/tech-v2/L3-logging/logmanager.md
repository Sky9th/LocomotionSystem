...
> `Utility/Logging/LogManager.cs` — 静态类，管理 LogChannel 注册、缓存和消息分发到所有 ILogAppender
...
- **备注**: `internal` 可见性，外部只能通过 LogChannel 方法间接调用
...
- appenders 有序列表，EmitInternal 依次遍历每个 appender
