...
> `Assets/Scripts/UI/Components/UIButton.cs` — 集成 IPointer 接口 + DOTween 动画 + 主题色。取代 Unity Button 原生 transition。
...
| 属性 | 类型 | 用途 |
|------|------|------|
| `Label` | string | 按钮文字 (get/set) |
| `Interactable` | bool | 是否可交互，disabled 时用 buttonDisabled 色 |
| `Button` | Button | 持有 Unity Button 引用 |
