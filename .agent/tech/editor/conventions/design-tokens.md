...
定义在 `EditorTokens`，所有 EditorWindow / EUI 组件引用：

| Token | 值 | 推导 | 用途 |
|-------|-----|------|------|
| `Pad` | `6f` | — | 标准间距——卡片内边距 = 卡片间隙 = 窗口边缘间距 |
| `PadTight` | `3f` | `Pad / 2` | 紧凑间距——Label 尾距、关联紧密元素间 |

> 所有组件的间距必须从 `Pad` 令牌推导（`Pad / 2`、`Pad * 2`），禁止硬编码 `3f`/`4f` 等魔术数字。
