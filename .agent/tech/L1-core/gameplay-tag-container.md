...
- 被各子系统（`AbilityComponent`、AI FSM、Stats、Item 等）持有，作为门控/冷却/状态标记的统一存储
- 全系统通用，每个实体持有一个独立实例
...
- **参数**: `query` — 查询字符串
- **返回**: 是否有标签匹配
- **调用者**: `AbilityComponent.TryActivate()` 检查门控标签；外部系统查询状态
...
- **调用者**: `AbilityComponent.TryActivate()` 检查 `activationBlockedTags`
