# StatInstance
> **源文件**: `Assets/Scripts/Stats/StatInstance.cs`

Stat 的运行时实例，持有当前值、修改器列表、Tick 分派逻辑。
...
| `ModifierCount` | `int` | 当前活跃修改器数量（计算属性） |
...
- **备注**: 绝对值变化小于 0.001 时不触发 OnChanged；Current 到达 Min 时触发 OnZero

## 内部机制

非 MonoBehaviour，纯 C# 类，不由 Unity 生命周期管理。由 CharacterStats 在构造时创建并持有。

### TickConsume(float dt)
- 如果 `consumeInterval > 0`：帧累加 consumeTimer，到达间隔后计算 ticks，公式：`(-consumeRate + Addend) * Multiplier * ticks`
- 如果 `consumeInterval == 0`：每帧执行，公式：`(-consumeRate + Addend) * Multiplier * dt`
- delta 为 0 时不调用 Modify
...
### CollectModifiers()
- 遍历所有 modifiers，调用每个 `m.Apply?.Invoke(this, ctx)`
...
| 规划 | 状态 | 依赖 | 来源 |
|------|------|------|------|
| 长间隔改事件驱动 | 远期 | TimeService | 代码 TODO (TickConsume) |
