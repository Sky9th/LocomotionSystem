# StatDefSO
> **源文件**: `Assets/Scripts/Stats/StatDefSO.cs`

ScriptableObject，定义单个 Stat 的类型、数值范围、能力勾选。
...
- 菜单路径: `Game/Stats/Stat Definition`
- 文件名默认值: `StatDef`

## 使用规则

- 能力通过 Inspector 勾选 + 填写速率字段，不通过接口实现
- 外部判断能力用 `Def.IsConsumable` / `Def.IsRestorable`，不直接读序列化字段
- 序列化字段 (`isConsumable`/`isRestorable`) 为小写开头，计算属性 (`IsConsumable`/`IsRestorable`) 为大写开头
