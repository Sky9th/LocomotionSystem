# StatsTreeSO
> **源文件**: `Assets/Scripts/Stats/StatsTreeSO.cs`

ScriptableObject，Stat 树的根入口，序列化嵌套结构，Resolve() 产出运行时 StatInstance 列表。
...
- 菜单路径: `Game/Stats/Stats Tree`
- 文件名默认值: `StatsTree`

Resolve 结果不缓存，每次调用重新构建整棵树。
