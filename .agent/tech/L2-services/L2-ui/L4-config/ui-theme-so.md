# UIThemeSO
> **源文件**: `Assets/Scripts/UI/Config/UIThemeSO.cs`

ScriptableObject。集中管理所有 UI 视觉参数：颜色风格、字体、字号、动画参数、StatBar 阈值。
...
所有配置参数均为 `public` 字段，通过 Inspector 或代码直接读写。详见下方 [配置参数](#配置参数) 表格。
...
每个包含 9 个 Color 字段（详见 UIColorSet struct）。
