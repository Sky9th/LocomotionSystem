# AudioSetSO
> **源文件**: `Assets/Scripts/Audio/AudioSetSO.cs`

抽象 ScriptableObject，供子模块继承扩展不同种类的音频集。
...
| 被依赖 | CharacterAudioConfigSO | 继承自 AudioSetSO |
| 被依赖 | FootstepSetSO | 继承自 AudioSetSO |
...
纯数据容器。继承 ScriptableObject 以利用 Unity 资源系统做配置序列化。
