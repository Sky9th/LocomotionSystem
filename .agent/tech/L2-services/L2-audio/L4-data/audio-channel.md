# AudioChannel
> **源文件**: `Assets/Scripts/Audio/AudioChannel.cs`

静态工具类，提供 AudioResponse 的播放能力。
...
| 方向 | 模块 | 关系 |
|------|------|------|
| 被依赖 | CharacterAudio | 消费方，调用 Play 方法 |
...
- **调用者**: 外部系统（爆炸、环境音等）
- **备注**: 使用 Unity 内置 AudioSource.PlayClipAtPoint，自动创建临时 AudioSource；不设置 Pitch
...
- 3D 重载不写入 pitch（PlayClipAtPoint 不支持 pitch 参数）
...
| 3D 播放支持 Pitch 参数 | 待做 | PlayClipAtPoint API 限制 |
