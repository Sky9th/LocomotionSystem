# IAPlayerSprint
> **源文件**: `Assets/Scripts/Inputs/Actions/Player/Button/IAPlayerSprint.cs`

继承 InputActionHandler。读取按钮状态 → 组装 SIActionSprint → 发布事件。
...
Unity Input System (Performed/Canceled)
  └── handler.Execute(context)
      ...
      ├── SIActionSprint.CreateEvent(isPressed, context.phase)
...
| 发布 | SIActionSprint | EventDispatcher 广播 |
| 消费 | 02-character (CharacterEventReceiver) | 订阅 SIActionSprint |
...
```
- **用途**: 读取按钮状态 → 组装 SIActionSprint → 发布事件
