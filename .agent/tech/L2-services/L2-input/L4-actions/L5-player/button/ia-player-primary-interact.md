# IAPlayerPrimaryInteract
> **源文件**: `Assets/Scripts/Inputs/Actions/Player/Button/IAPlayerPrimaryInteract.cs`

继承 InputActionHandler。读取按钮状态 → 组装 SIActionPrimaryInteract → 发布事件。
...
Unity Input System (Performed/Canceled)
  └── handler.Execute(context)
      ...
      ├── SIActionPrimaryInteract.CreateEvent(isPressed, context.phase)
...
| 发布 | SIActionPrimaryInteract | EventDispatcher 广播 |
| 消费 | 02-character (CharacterEventReceiver) | 订阅 SIActionPrimaryInteract |
...
```
- **用途**: 读取按钮状态 → 组装 SIActionPrimaryInteract → 发布事件
