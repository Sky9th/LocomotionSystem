# IAPlayerStand
> **源文件**: `Assets/Scripts/Inputs/Actions/Player/Button/IAPlayerStand.cs`

继承 InputActionHandler。读取按钮状态 → 组装 SIActionStand → 发布事件。
...
Unity Input System (Performed/Canceled)
  └── handler.Execute(context)
      ...
      ├── SIActionStand.CreateEvent(isPressed, context.phase)
...
| 发布 | SIActionStand | EventDispatcher 广播 |
| 消费 | 02-character (CharacterEventReceiver) | 订阅 SIActionStand |
...
```
- **用途**: 读取按钮状态 → 组装 SIActionStand → 发布事件
