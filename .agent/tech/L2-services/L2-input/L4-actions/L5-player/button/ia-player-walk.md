# IAPlayerWalk
> **源文件**: `Assets/Scripts/Inputs/Actions/Player/Button/IAPlayerWalk.cs`

继承 InputActionHandler。读取按钮状态 → 组装 SIActionWalk → 发布事件。
...
Unity Input System (Performed/Canceled)
  └── handler.Execute(context)
      ...
      ├── SIActionWalk.CreateEvent(isPressed, context.phase)
...
| 发布 | SIActionWalk | EventDispatcher 广播 |
| 消费 | 02-character (CharacterEventReceiver) | 订阅 SIActionWalk |
...
```
- **用途**: 读取按钮状态 → 组装 SIActionWalk → 发布事件
