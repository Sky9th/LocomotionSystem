# IAPlayerProne
> **源文件**: `Assets/Scripts/Inputs/Actions/Player/Button/IAPlayerProne.cs`

继承 InputActionHandler。读取按钮状态 → 组装 SIActionProne → 发布事件。
...
Unity Input System (Performed/Canceled)
  └── handler.Execute(context)
      ...
      ├── SIActionProne.CreateEvent(isPressed, context.phase)
...
| 发布 | SIActionProne | EventDispatcher 广播 |
| 消费 | 02-character (CharacterEventReceiver) | 订阅 SIActionProne |
...
```
- **用途**: 读取按钮状态 → 组装 SIActionProne → 发布事件
