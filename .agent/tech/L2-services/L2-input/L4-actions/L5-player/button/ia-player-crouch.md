# IAPlayerCrouch
> **源文件**: `Assets/Scripts/Inputs/Actions/Player/Button/IAPlayerCrouch.cs`

继承 InputActionHandler。读取按钮状态 → 组装 SIActionCrouch → 发布事件。
...
Unity Input System (Performed/Canceled)
  └── handler.Execute(context)
      ...
      ├── SIActionCrouch.CreateEvent(isPressed, context.phase)
...
| 发布 | SIActionCrouch | EventDispatcher 广播 |
| 消费 | 02-character (CharacterEventReceiver) | 订阅 SIActionCrouch |
...
```
- **用途**: 读取按钮状态 → 组装 SIActionCrouch → 发布事件
