# LogChannel
> **源文件**: `Assets/Scripts/Utility/Logging/LogChannel.cs`

带级别过滤的命名日志频道，每个模块/系统独立实例。
...
- **参数**: `message` — 日志文本
- **调用者**: 关键事件标记（系统启动、关闭等）
- **备注**: 直接调 `LogManager.EmitInternal(..., bypass: true)`，不经过 IsEnabled 检查；bypass 时仍受 appender.Threshold 限制
...
- 不对外公开构造，必须通过 `LogManager.GetChannel()` 创建
- 子频道通过 GetChild 创建，命名自动继承父级命名空间

## 未来规划

| 规划 | 状态 | 来源 |
|------|------|------|
| 频道级别动态重载 — 运行时从配置文件刷新 | 待做 | 旧 logging-system.md |
