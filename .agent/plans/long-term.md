...
> 更新: 2026-05-23
> 来源: `.agent/design/` GDD + 4 个新子系统设计（伤病/噪音/负重/死亡）
...
设计完成:
  GDD (game-overview) ✅   伤病系统 ✅   噪音系统 ✅
  负重/背包 ✅             死亡/存档 ✅

已具备: 角色移动、脚步音效、HP/饥饿/口渴/体力 实时消耗与 HUD 显示、
        主菜单→游戏→暂停 完整 UI 闭环、6 属性 + 16 熟练度数据结构就绪
```
...
> 设计文档: `injury-system.md` / `noise-system.md`

| 子系统 | 基本闭环 |
|--------|----------|
| 战斗 | 近战武器 SO → 鼠标左键攻击 → 射线检测 → 伤害判定 → DamageRule 入口 → 施加割伤/钝器伤/咬伤 |
| 敌人 | 丧尸闲逛 → 听觉感知（噪音半径）→ 视觉感知（视线锥）→ Idle/Alerted/Chase/Attack FSM → 被击杀 → 基础掉落 |
| 噪音 | 移动/战斗/交互 发布 SNoiseEvent（等级+位置+类型）→ 丧尸在半径内 → Alerted |
| 伤病 | 攻击命中 → 根据武器类型施加伤害（割伤/钝器伤/咬伤）→ HP 扣除 + 流血/疼痛/感染 |

**可玩增量**: 地图上有丧尸→你走路发出声音→丧尸听到→追你→你用斧头砍伤它→它流血→死亡→掉落物品。

**不做的**: 远程武器、技能栏、熟练度成长、噪音连锁反应（第 2 层）、障碍物噪音衰减、丧尸化过程（仅做感染值累积）。
...
**不做的**: 六大类完整分类、仓库系统、工具耐久/维修、大背包/军用背包（仅做基础背包+小背包）。
...
| 阶段 | 内容 | 预估 |
|------|------|------|
| Phase 4 | 战斗 + 敌人 AI 基础 | 当前 |
| Phase 5 | 资源系统 | 2026-Q3 |
| Phase 6-7 | 建造 + 时间日夜 | 2026-Q4 |
| Phase 8-9 | 农业烹饪 + NPC | 2027-Q1 |
| Phase 10-11 | 尸潮 + 科技树 | 2027-Q2 |
| Phase 12+ | 扩展打磨 | 2027-Q3+ |
