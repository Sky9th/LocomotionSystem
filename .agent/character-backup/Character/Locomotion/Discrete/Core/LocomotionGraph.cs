...
namespace Game.Locomotion.Discrete.Core
{
    ...
    internal sealed class LocomotionGraph
    {
        ...
        /// </summary>
        public SLocomotionDiscrete Evaluate(in SCharacterKinematic kinematic, in SLocomotionMotor motor, in SCharacterInputActions actions)
        {
            phaseAspect.Update(in kinematic, in motor, in actions);
            gaitAspect.Update(in kinematic, in motor, in actions);
            postureAspect.Update(in kinematic, in motor, in actions);
