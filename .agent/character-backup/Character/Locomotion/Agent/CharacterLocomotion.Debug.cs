...
{
    /// <summary>
    /// Debug and gizmo rendering logic for <see cref="CharacterLocomotion"/>.
    /// Kept in a separate partial class to keep the core agent logic
    /// focused on simulation and state management.
    /// </summary>
    public partial class CharacterLocomotion
    {
        [Header("Debug")]
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            if (snapshot.Kinematic.GroundContact.ContactPoint != Vector3.zero)
            {
                GizmoDebugUtility.DrawSphere(snapshot.Kinematic.GroundContact.ContactPoint, 0.05f, Color.green, "Ground Contact Point");
            }
            /// Ground Debug end
            ...
            if (snapshot.Kinematic.ForwardObstacleDetection.HasHit)
            {
                GizmoDebugUtility.DrawSphere(snapshot.Kinematic.ForwardObstacleDetection.Point, 0.05f, Color.magenta, "Obstacle Hit Point");
                GizmoDebugUtility.DrawArrowLine(
                    snapshot.Kinematic.ForwardObstacleDetection.Point,
                    snapshot.Kinematic.ForwardObstacleDetection.Point + snapshot.Kinematic.ForwardObstacleDetection.Normal * 0.3f,
                    Color.red,
                    "Obstacle Hit Normal");

                if (snapshot.Kinematic.ForwardObstacleDetection.HasTopSurface)
                {
                    Vector3 topProbeOrigin = snapshot.Kinematic.ForwardObstacleDetection.Point + obstacleForward * 0.05f;
                    topProbeOrigin.y = transform.position.y + Profile.obstacleMaxClimbHeight + 0.05f;

                    GizmoDebugUtility.DrawArrowLine(
                        topProbeOrigin,
                        snapshot.Kinematic.ForwardObstacleDetection.TopPoint,
                        Color.white,
                        "Obstacle Height Probe");
                    GizmoDebugUtility.DrawSphere(snapshot.Kinematic.ForwardObstacleDetection.TopPoint, 0.05f, Color.green, "Obstacle Top Point");
                }
            }
            /// Obstacle Debug end
        }
