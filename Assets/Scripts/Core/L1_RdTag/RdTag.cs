...
namespace RedDust.Core.RdTag
{
    ...
    /// 设计时使用 <see cref="RdTagDefSO"/> 定义，运行时通过隐式转换获取。
    ///
    /// 层级匹配：查询 "State" 匹配 "State.Attacking"（前缀+点），
    /// 但不匹配 "StateAttacking"（缺少层级分隔符）。
    /// </summary>
    public readonly struct RdTag : IEquatable<RdTag>
    {
        ...
        public bool Matches(RdTag query) => Matches(query.Tag);
        ...
        public bool IsDescendantOf(RdTag other) => other.IsAncestorOf(this);

        // ── Equality（HashSet 去重 + O(1) 查询）──

        public bool Equals(RdTag other) => Tag == other.Tag;
        public override bool Equals(object obj) => obj is RdTag other && Equals(other);
        public override int GetHashCode() => Tag?.GetHashCode() ?? 0;

        public static bool operator ==(RdTag a, RdTag b) => a.Tag == b.Tag;
        public static bool operator !=(RdTag a, RdTag b) => a.Tag != b.Tag;
