...
namespace RedDust.Core
{
    ...
    /// 设计时使用 <see cref="rTagDefSO"/> 定义，运行时通过隐式转换获取。
    ///
    /// 层级匹配：查询 "State" 匹配 "State.Attacking"（前缀+点），
    /// 但不匹配 "StateAttacking"（缺少层级分隔符）。
    /// </summary>
    public readonly struct rTag : IEquatable<rTag>
    {
        ...
        public bool Matches(rTag query) => Matches(query.Tag);

        /// <summary>this 是否是 other 的祖先层级（前缀）。</summary>
        public bool IsAncestorOf(rTag other)
            => !string.IsNullOrEmpty(Tag)
               && other.Tag != null
               && other.Tag.StartsWith(Tag + ".", StringComparison.Ordinal);

        /// <summary>this 是否是 other 的后代层级。</summary>
        public bool IsDescendantOf(rTag other) => other.IsAncestorOf(this);

        // ── Equality（HashSet 去重 + O(1) 查询）──

        public bool Equals(rTag other) => Tag == other.Tag;
        public override bool Equals(object obj) => obj is rTag other && Equals(other);
        public override int GetHashCode() => Tag?.GetHashCode() ?? 0;

        public static bool operator ==(rTag a, rTag b) => a.Tag == b.Tag;
        public static bool operator !=(rTag a, rTag b) => a.Tag != b.Tag;
