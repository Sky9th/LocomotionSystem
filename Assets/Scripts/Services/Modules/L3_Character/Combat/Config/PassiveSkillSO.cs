...
namespace RedDust.Character.Combat
{
    ...
    /// 与主动技能共享 GameplayEffectSO 体系，但不走 activation/search/noise/combo 管道。
    ///
    /// 两种触发路径：
    /// - 常用: trigger 枚举，CombatComponent 内部流程节点直接匹配（OnHit/OnKill/OnDamaged/OnDodge/OnComboStage/OnEquip）
    /// - 罕用: triggerChannel 引用外部 EventChannel 资产（时间/天气/Boss阶段），非 null 时覆盖枚举匹配
    /// </summary>
    [CreateAssetMenu(menuName = "RedDust/Combat/Passive Skill", fileName = "Passive_")]
