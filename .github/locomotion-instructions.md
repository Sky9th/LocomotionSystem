...
- 各类表现 Adapter（例如 `LocomotionAnimancerAdapter`）只读 Agent/Manager 提供的快照，驱动 Animator、音频、特效等表现层。
...
2. **LocomotionAgent（每角色挂载）**
   ...
   - 通过序列化字段引用 `LocomotionConfigProfile`，所有移动速度、加速度、头部旋转限制、转身阈值等行为调参均从 Profile 读取；Agent 本身不再内建数值默认值，未挂配置视为错误配置。

3. **动画 / 表现 Adapter（参数与表现桥接器）**
   - 独立 MonoBehaviour，作为 LocomotionAgent 的伴随组件，**只读** Agent 推送的 `SPlayerLocomotion` 快照，把速度、`IsTurning`、`TurnAngle`、头部 Look 等信息映射到 Animancer 动画状态、VFX、音频或 UI；不负责生成/修改快照。
   - 暴露序列化字段以配置 Animancer 状态或 Transition 资源引用、混合权重/平滑曲线、阈值触发（如落地、加速），并在 `Update`/`LateUpdate` 中执行同步，确保表现调参与核心物理解耦。
   - 当前实现阶段仅提供动画适配器 `LocomotionAnimancerAdapter`，用于驱动 Animancer 层与头部 Look 混合；后续如有需要再按需增加脚步音、屏幕震动等专用 Adapter，而不是预先引入通用基类。
...
- 动画 Adapter（例如 `LocomotionAnimancerAdapter`）必须与这些状态保持一致：速度来自 `SPlayerLocomotion.Velocity.magnitude`，`IsTurning/TurnAngle` 由 Agent 的朝向差计算；触发时机务必与 Animancer 状态切换条件同步，避免逻辑/表现错位。
...
9. **配置资产**
   - `LocomotionConfigProfile` ScriptableObject 保存基础移动速度、加速度、头部旋转约束、转身进入/完成阈值、抖动容差时间等行为调参，是 `LocomotionAgent` 的必选依赖。
   - 可选曲线（后续）：加速度-坡度、平滑 Blend 时间等，可按需要扩展到 Profile 内。
   - 通过序列化字段注入 LocomotionAgent，每个角色可以引用不同 Profile 以获得不同移动手感；Manager 仍保持无状态，仅消费 Agent 推送的快照。
