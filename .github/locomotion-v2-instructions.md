...
**重要约束（表现数据契约）：**
- Animation 模块只允许“应用”上游计算出来的结果，不允许在动画侧根据 `TurnAngle`/`turnSpeed`/`deltaTime` 等再做积分或步进计算。
- 因此，若动画需要对 ModelRoot 应用本帧旋转增量，必须由 Agent/State/Computation 提前算出并写入快照，例如：
  - `float ModelYawDeltaDegrees; // 本帧建议应用到模型的Yaw增量（度）`
  - Animation 只读该字段并选择是否应用（例如播放 root motion 转身片段时可抑制应用）。
...
- `TurnCalculator`：
- `TurnCalculator`：
  - 职责：基于 Model 朝向和 Follow 水平向量，计算转身角度、是否需要原地转身等。
  - 输出应包含“可直接被动画应用”的结果（而不仅是原始角度），例如：
    - `ModelYawDeltaDegrees`：本帧建议应用到模型的Yaw增量（度，已包含 dt 与 clamp）。
...
**目的：**
- 只读 `SPlayerLocomotion` 等快照 + 动画配置，驱动 Animancer；
- 实现动画表现（播什么、怎么混、是否应用某些表现增量），但不参与 locomotion 物理/规则计算。

**强制约束（不做计算）：**
- Animation 不允许根据 `TurnAngle`、`turnSpeed`、`deltaTime` 等进行任何积分、步进、平滑、滤波等“计算逻辑”。
- Animation 仅允许“应用”上游已经算好的结果：
  - 例如快照中提供的 `ModelYawDeltaDegrees`（本帧 yaw 增量）。
  - Animation 可以基于“表现策略”选择是否应用（例如 root motion 转身片段播放期间抑制应用），但不得重新计算增量。
...
- `LocomotionAnimationContext`
  ...
  - 结构体，封装驱动每一帧动画层所需的信息：
    ...
    - （可选）`ILocomotionModelRotator ModelRotator`
  - 作为所有动画层的统一只读输入，屏蔽 MonoBehaviour 细节，便于单元测试。

> 说明：`ModelRotator` 仅用于“应用快照中给出的表现增量”（例如 `ModelYawDeltaDegrees`）到 ModelRoot。
> Animation 不应在此处进行任何角速度/步进计算。
...
##### 4.3.1 动画 FSM（强制文件组织约定）

当动画需求明确为“每个动画（含 Mixer）必须由 FSM 管理”时，Animation/Layers 的文件组织与职责必须遵循以下约定，以避免单文件膨胀并支持渐进迁移：

**推荐目录结构：**

- `Assets/Scripts/LocomotionV2/Animation/Layers/`
  - `Shared/`
    - `Fsm/`（可选）
      - 放置跨 Layer 复用的 FSM 基类与工具（例如：完成判定策略、统一 Tick 接口、播放封装等）
  - `<LayerName>/`
    - `Fsm/`
      - `<LayerName>LayerFsm.cs`（该 Layer 的 FSM 入口/编排器，强制保持短小）
      - `States/`
        - `*.cs`（该 Layer 的全部 State，通常 1 State 1 文件）
      - `Conditions/`（建议）
        - 该 Layer 私有的条件复用（只返回 bool，不做播放/不做计算）。
      - `Appliers/`（建议）
        - 该 Layer 私有的“应用器”复用（只把快照结果应用到 Animancer/ModelRotator，不做计算）。

**Appliers/ 约定（强制）：**
- Applier 只做“Apply”，不做任何 locomotion 计算：
  - 禁止：根据 `TurnAngle`、`deltaTime`、`turnSpeed`、阈值等推导本帧增量；
  - 允许：将快照中“已计算好的结果字段”直接写入表现对象（Animancer 参数 / Layer 权重 / ModelRotator 等）。
- Applier 只允许做最小防护：
  - 目标对象为空、增量为 0、参数未命中等情况的早退。
- 命名：`*Applier.cs`，一个 Applier 聚焦一个“写入目标”。

**Appliers/ 示例文件（建议）：**
- `ModelYawDeltaApplier.cs`
  - 输入：`ILocomotionModelRotator`、`float modelYawDeltaDegrees`
  - 行为：当 `abs(modelYawDeltaDegrees) > epsilon` 时，调用 `rotator.RotateModelYaw(modelYawDeltaDegrees)`
  - 数据来源：`context.Snapshot.ModelYawDeltaDegrees`（由 Agent/State/Computation 写入）
- `MixerParameterApplier.cs`
  - 输入：某个 `Vector2MixerState`/`FloatMixerState` 与快照中的“已准备好的参数值”
  - 行为：直接写 `mixer.Parameter = value`
  - 注意：如果参数需要复杂映射/归一化/滤波，应由上游写入快照结果字段；动画侧仅应用。
    - `Legacy/`
      - 旧实现仅用于参考（不删除、不修改行为；新实现与旧实现并行存在）

**职责划分（强制）：**

- `*LayerFsm.cs`（Layer 入口/编排器）只负责：
  - 组装并持有 FSM（建议使用 `Animancer.FSM`）；
  - 每帧接收 `LocomotionAnimationContext` 并驱动 FSM；
  - 统一封装播放（例如 `PlayIfChanged` / `TryPlay`）与输出 `SLocomotionAnimationLayerSnapshot`；
  - 若存在 Mixer 参数更新：入口只负责“调度”。
    - 若参数为“上游已计算的结果”（推荐），State 只负责应用；
    - 若参数为纯表现映射（例如直接把快照中的速度/方向映射到混合参数），可以在 State 内实现，但禁止做与 locomotion 规则相关的积分/平滑/步进计算。

- `States/*.cs`（State 文件）只负责本 State 的逻辑：
  - `CanEnterState` / `CanExitState` / `OnEnterState` / `OnExitState`；
  - 额外约定 `Tick()`（或 `Tick(in LocomotionAnimationContext context)`），由 Layer 入口每帧调用。

**命名规范：**

- Layer：`<Feature>LayerFsm`（例如 `BaseLayerFsm`）。
- State：`<Layer><Action>State` 或 `<Action>State`；但在同一 `States/` 下必须保证唯一且可读。

**迭代约束（建议）：**

- 每次迭代优先新增/调整 1~3 个 State，并确保可验证的最小行为闭环（避免一次性铺开所有 State）。
