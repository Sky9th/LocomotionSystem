...
## 概述
GameContext 常驻于 GameManager 的子层级，是场景内“运行期上下文”的唯一可信入口。所有需要共享访问的运行时数据（摄像机状态、玩家姿态、天气/AI 反馈、服务入口等）都应以 Struct 快照形式注册到 GameContext，避免脚本间随意互相引用造成耦合。
...
## 数据分层
- **Struct Snapshot Layer**：仅缓存结构体快照，例如 `CameraContextStruct`（位置、前向、近远裁剪等）、`PlayerPoseStruct`、`WeatherContextStruct`。每个子系统负责在自身更新点调用 `GameContext.UpdateSnapshot(struct)` 写入最新状态，消费方通过 `TryGetSnapshot<TStruct>` 读取上一帧稳定值。
...
## 协作要点
- **输入/摄像机/AI 子系统**：各自维护专属 Struct（例如 `PlayerMoveIntentStruct`、`CameraContextStruct`、`EnemyAwarenessStruct`），并在更新后调用 `UpdateSnapshot` 推送；需要数据的系统通过 `TryGetSnapshot` 读取。
...
## 扩展建议
- 定义 `CameraContextStruct`、`PlayerPoseStruct` 等专用结构，与 Shared Structs 约定保持一致。
- 增设 `ActorRegistry`、`WeatherContextStruct` 等结构化数据，让共享状态都以 Struct 形式存在。
- 若接入多人或网络，可在 GameContext 内维护 `Dictionary<PlayerId, PlayerContextStruct>` 并周期性推送快照。
